...
対応する注釈は、単一C++ソースのグローバルな1行・1変数の数値リテラル初期化です。型は`int`・`long long`・`float`・`double`で、`static`・`const`・`constexpr`を付けられます。`step=100`による刻み指定、`log`による対数探索に対応します。型エイリアス・式による初期化・関数内・クラス内・条件付きプリプロセッサ内の注釈は診断します。

pahcerの`compile_steps`と`test_steps`は記載順のまま使用します。`g++`、`clang++`、`make`、`ccache`、Cargo、プロジェクト内のビルドスクリプトなど、シェルを介さず`program`と`args`で実行できる構成に対応します。`current_dir`とプロジェクト内コマンドの参照先はworkspace内に限定されます。設定した`program`は従来のRunと同様に利用者の権限で実行されるため、信頼できるローカルのpahcer設定だけを使用してください。
...
Study開始時にプロジェクトのヘッダー、Makefile、スクリプトなどを固定コピーし、各Trialはそのコピーに候補ソース、tools、入力、pahcer設定を重ねて実行します。`.git`、`ahc-plaza`、`pahcer`、`build`、`target`、`dist`、設定済みの入力rootとtoolsはprojectコピーから除外します。外部コマンドは解決先と実行ファイルのSHA-256をmanifestへ記録し、変更後の再開を拒否します。
